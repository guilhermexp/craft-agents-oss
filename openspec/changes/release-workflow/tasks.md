...
- [ ] 1.1 Criar `.github/workflows/release.yml` com:
...
- [ ] 1.2 Job `build` com `strategy.matrix.include`:
...
- [ ] 1.3 Steps comuns a todos os jobs (na ordem):
...
- [ ] 1.4 Step `Build artifacts` com `env.CSC_IDENTITY_AUTO_DISCOVERY: "false"`:
  - macOS: `bun run electron:dist:mac` (arquitetura nativa do runner)
  - Windows: `bun run electron:dist:win`
- [ ] 1.5 Step `Upload to release draft` usando `softprops/action-gh-release@v2`:
  - `draft: true`
...
- [ ] 2.1 Após commit do workflow, criar tag de teste: `git tag v0.0.0-rc.workflow-test && git push origin v0.0.0-rc.workflow-test`
- [ ] 2.2 Observar execução em `gh run watch`. Se falhar em `bundle:hermes` ou downloads vendor, adicionar step explícito antes (ex: `setup-python` ou `pwsh` actions). Documentar fix.
- [ ] 2.3 Confirmar artefatos aparecem na Release draft com nomes esperados.
- [ ] 2.4 Deletar tag e draft de teste após validar.

## 3. Limpeza package.json (raiz)

- [ ] 3.1 Remover do bloco `scripts` em `/Users/guilhermevarela/Documents/Projetos/SelfHosting/craft-agents-oss/package.json`:
  - `"release": "bun run scripts/release.ts"`
  - `"check-version": "bun run scripts/check-version.ts"`
  - `"oss:sync": "bun run scripts/oss-sync.ts"`
- [ ] 3.2 Confirmar via `bun install` que lockfile não muda (são scripts, não deps)
- [ ] 3.3 Confirmar `bun run validate:ci` continua passando

## 4. Doc RELEASING.md

- [ ] 4.1 Criar `/Users/guilhermevarela/Documents/Projetos/SelfHosting/craft-agents-oss/RELEASING.md` com seções:
...
- [ ] 4.2 Linkar `RELEASING.md` a partir do `CONTRIBUTING.md` existente

## 5. Validação final

- [ ] 5.1 Rodar `openspec validate release-workflow --strict --no-interactive` — verde
...
- [ ] 5.3 Confirmar critérios verificáveis do `design.md` seção "Critério verificável de done":
  - Release draft gerada via dry-run (já feito em 2.x)
  - 5 artefatos esperados presentes
  - package.json sem scripts fantasma
  - RELEASING.md existe e linka pra signing changes futuras
- [ ] 5.4 Reportar: paths criados/modificados, link da Release draft de teste, saída de `openspec list`
