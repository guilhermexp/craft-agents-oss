...
import { RPC_NAMESPACES, type WindowCloseRequestSource } from '../shared/types'
...
export class WindowManager {
  ...
  createWindow(options: CreateWindowOptions): BrowserWindow {
      ...
      window.once('ready-to-show', () => {
        ...
        import('./deep-link').then(({ parseDeepLink }) => {
          ...
          if (target && (target.view || target.action)) {
            ...
            setTimeout(() => {
              this.pushToWindow(window, RPC_NAMESPACES.deeplink.NAVIGATE, {
                view: target.view,
    ...
    const themeHandler = () => {
      this.pushToWindow(window, RPC_NAMESPACES.theme.SYSTEM_CHANGED, nativeTheme.shouldUseDarkColors)
    }
    nativeTheme.on('updated', themeHandler)

    // Handle focus/blur to broadcast window focus state
    window.on('focus', () => {
      this.pushToWindow(window, RPC_NAMESPACES.window.FOCUS_STATE, true)
    })
    window.on('blur', () => {
      this.pushToWindow(window, RPC_NAMESPACES.window.FOCUS_STATE, false)
    })
    ...
    window.on('close', (event) => {
      ...
      if (!window.webContents.isDestroyed() && window.webContents.mainFrame) {
        ...
        this.pushToWindow(window, RPC_NAMESPACES.window.CLOSE_REQUESTED, { source })
