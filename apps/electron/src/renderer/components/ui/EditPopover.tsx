  ...
  'edit-channels': (location) => ({
    context: {
      ...
      context:
        ...
        'Channels may also define participants for war-room style agent calls. Each participant has id, displayName, llmConnection, and optional model, hermesProfile, defaultSourceSlugs, permissionMode, workingDirectory. For multiple Hermes profiles in the same channel, use llmConnection "hermes" for each participant and set hermesProfile to the profile slug, for example { id: "server-ops", displayName: "Server Ops", llmConnection: "hermes", hermesProfile: "server-ops" }. The preferred War Room mode is routing.mode "orchestrator" with routing.leadParticipantId set to the Hermes profile that should decompose, delegate through Hermes Kanban/gateway, and consolidate worker results back into the channel. ' +
        'routing.mode controls default dispatch: "orchestrator" routes all channel messages to the lead with a worker roster, "manual-tags" only dispatches explicit @mentions, "lead" routes unmentioned messages to routing.leadParticipantId, and "all" dispatches to every participant. ' +
