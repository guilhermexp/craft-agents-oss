 ...
 * response-card.tsx
 *
 * ResponseCard — the streamed assistant/plan message card plus its inline text
 * annotation island (selection → annotate → follow-up). Extracted from
 * TurnCard.tsx; the buffering decision now lives in ./streaming-buffer.
 */
import * as React from 'react'
import { useMemo, useEffect, useEffectEvent, useRef, useCallback, useState } from 'react'
import { useTranslation } from 'react-i18next'
import type { AnnotationV1 } from '@craft-agent/core'
import { FileText, Copy, Check, Maximize2, ListTodo, GitBranch } from 'lucide-react'
...
import { type PointerSnapshot, buildAnnotationChipEntryTransition, buildSelectionEntryTransition } from '../annotations/island-motion'
import { extractAnnotationSelectedText } from './follow-up-helpers'
import { formatAnnotationFollowUpTooltipText, getAnnotationNoteText } from '../annotations/follow-up-state'
import { ANNOTATION_PREFIX_SUFFIX_WINDOW, SELECTION_POINTER_MAX_AGE_MS, clamp, hasExistingTextRangeAnnotation, createSelectionPreviewAnnotation, createTextSelectionAnnotation, collectTextSegments, getCanonicalText, resolveNodeOffset, type AnnotationOverlayRect } from '../annotations/annotation-core'
import { annotationColorToCss } from '../annotations/annotation-style-tokens'
...
import { getAnnotationInteractionAnchor, getAnnotationInteractionSourceKey, hasAnnotationInteraction } from '../annotations/interaction-selectors'
import { type AnnotationIslandMode } from '../annotations/interaction-state-machine'
...
import { SIZE_CONFIG, type AnnotationInteractionMode, type OpenAnnotationRequest } from './turn-card-shared'
import { shouldShowContent, BUFFER_CONFIG } from './streaming-buffer'
