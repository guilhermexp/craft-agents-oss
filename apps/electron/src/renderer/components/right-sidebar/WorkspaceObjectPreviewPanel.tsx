...
export function WorkspaceObjectPreviewPanel({ workspaceId, objectId }: { workspaceId: string; objectId: string }) {
  ...
  React.useEffect(() => {
    ...
    void resolver.load(target, fetchPayload).then(value => {
      ...
      if (!(error instanceof DOMException && error.name === 'AbortError')) console.error('[WorkspaceObjectPreview]', error)
    })
    ...
    return () => { active = false; resolver.invalidate(target); unsubscribe() }
  ...
  if (!payload) return <div className="p-4 text-xs text-muted-foreground">{t('chat.workspaceObjectPreviewLoading')}</div>
  return (
    <div className="h-full min-h-0 overflow-auto p-3">
      ...
      <DataTable columns={columns} data={payload.entries} pagination pageSize={50} />
    </div>
  )
}
