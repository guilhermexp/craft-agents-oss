...
- **A pending steer rides the same hook output, deny included.** `canDeliverSteer()` (`claude-agent.ts`) decides who carries `pendingSteerMessage` as `hookSpecificOutput.additionalContext`: every outcome that keeps the turn alive — `allow`, `modify`, `passthrough` and a `deny` without `endTurn` (`encodeClaudeToolBlock(result, steerContext)`). The `endTurn` shape must **not** consume it: the loop dies right after the hook, and leaving it pending is what makes `chatImpl`'s `finally` emit `steer_undelivered` so the session layer re-queues the message. Consuming the steer before the switch (the pre-fix shape) dropped the user's mid-turn message silently on every block, and with the turn now surviving N tool calls that is the common path, not a rare one.
- **The prerequisite deadlock escape is per turn.** `PrerequisiteManager.MAX_REJECTIONS` (`agent/core/prerequisite-manager.ts`) was calibrated when one block ended the turn, so releasing on the second call meant a human had resent the message. With the turn alive the model emits both calls itself, so the budget is now 3 **and** re-armed by `beginTurn()` at the top of `BaseAgent.chat()` (Claude and Pi). Only `rejectionCounts` re-arms: `readFiles`/`pendingSkillPaths` are session-lived and dropped solely by `resetReadState()` on compaction. The escape exists for the model that *cannot* comply (guide unreadable/vanished, Read disabled) — denying forever burns the turn in a tool-call loop with no assistant text. `strict` rules (browser docs) never reach the fallback, and the skill arm charges the budget to one pending path and releases only that path instead of `pendingSkillPaths.clear()`. Guarded by `agent/core/__tests__/prerequisite-manager.isolated.ts`.
- A turn that ends without an assistant message is never silent **unless the stop was intentional**: `SessionManager`'s `complete` handler (`packages/server-core/src/sessions/SessionManager.ts`) keeps the captured-400 `image_too_large`/`invalid_request` path and otherwise emits a generic retryable `unknown_error` ("No Response"), but only when `shouldReportMissingAssistantResponse()` agrees — `cancelProcessing` leaves `isProcessing` true so events drain, so Stop and redirect reach that branch with `stopRequested`/`wasInterrupted` set and a queued message awaiting replay; a red retry card there is a false positive on top of "Response interrupted". The SDK `result.stop_reason` is **not** plumbed into that branch — `AgentEvent`'s `complete` variant (`packages/core/src/types/message.ts`) carries no reason field, so a per-turn stop reason would need a new field across core types, the Claude event adapter and the session layer.
