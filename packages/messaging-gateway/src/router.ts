 ...
 *
 * Looks up the ChannelBinding for (platform, messagingChannelId).
...
export class Router {
  ...
  async route(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    ...
    const binding = this.bindingStore.findByChannel(msg.platform, msg.messagingChannelId, msg.threadId)
