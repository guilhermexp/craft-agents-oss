...
describe('content tabs state', () => {
  ...
  test('gives every opened file its own tab, and reopening one selects it back', () => {
    ...
    state = contentTabsReducer(state, { type: 'open', target: file('three.md'), mode: 'permanent' });

    expect(state.tabs.map(tab => tab.target.kind === 'file' ? tab.target.path : '')).toEqual(['one.md', 'two.md', 'three.md']);

    // The second click of a double-click reopens what is already there. It must
    // select, never duplicate - the id is derived from the target, not the open.
