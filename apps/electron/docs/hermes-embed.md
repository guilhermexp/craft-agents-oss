...
| `09-meet-playwright-on-demand.patch` | `plugins/google_meet/tools.py` + new `plugins/google_meet/_craft_playwright.py` — Craft no longer vendors the Playwright driver (~128 MB). Before the first local Google Meet join, `handle_meet_join` calls `ensure_playwright_available()`, which downloads Playwright (+ `websockets`) and Chromium **on demand** into an app-scoped writable dir (`$HERMES_HOME/runtime-deps/google-meet`), registers it on `sys.path`/`PYTHONPATH` (so the spawned `meet_bot` subprocess resolves it) and points `PLAYWRIGHT_BROWSERS_PATH` there. Progress streams to stderr (never stdout, which carries ACP JSON-RPC); an offline first use fails with a clear error instead of a bare `ImportError` crash. The signed venv is never mutated. |
| `10-craft-browser-skill-xai-web-search.patch` | `model_tools.py`, `tools/skills_tool.py`, `tools/web_tools.py` — normalizes legacy Hermes `browser_*` calls to Craft's canonical `mcp__session__browser_tool` when the Craft session MCP tool is present, makes ambiguous `hermes-agent` skill loads prefer the `agent-ops` category, and uses xAI Responses `web_search` as a credential-backed fallback for `web_search` provider failures. |
...
| `packages/shared/src/mcp/session-tools-server.ts` | Craft-native tools exposed to Hermes include browser, delegation/session, LLM, auth/config helpers, metadata, and automation; callbacks stay session-scoped. Every tool the bridge lists is derived from the canonical `defineTool(...)` catalog in `packages/session-tools-core/src/tool-defs.ts` — the bridge no longer declares any schema of its own. `automation_tool` and `meeting_tool` are `defineTool` entries carrying `exposure: 'mcp-only'` (they are reachable through this bridge but not through the Claude/Pi native adapters); the checker's former `craftBridgeOnly` escape hatch is gone, so a tool that skips `defineTool` fails `bun run lint:tool-contracts` instead of being waved through. Loopback requests are guarded against DNS rebinding (`Host` must be loopback; `Origin`, when present, must be loopback — see `loopback-guard.ts`); native MCP clients send no web `Origin` and pass unchanged. An opt-in `authToken` (bearer) option exists but is off by default — enabling it for Hermes requires first validating that the Hermes MCP client forwards headers configured via ACP `session.mcpServers[].headers`. |
| `packages/server-core/src/handlers/rpc/hermes.ts` | Thin protocol adapter only: ~24 one-line delegations; the channel list derives from `RPC_NAMESPACES.hermes` (no local `HANDLED_CHANNELS` array). All Hermes runtime behaviour — runtime detection, dashboard process launch, file/log/skill browsing, dashboard-delegated dev update env, update marker watching, env CRUD, and restart notification — lives in `packages/server-core/src/hermes/hermes-runtime-manager.ts` and stays local-only and path-safe under app-scoped `HERMES_HOME`. The manager also watches `<HERMES_HOME>/auth.json` so that when Hermes refreshes a Codex (`openai-codex`) OAuth token the new tokens are written back into Craft's credential store via `setLlmOAuth('chatgpt-plus', …)`. In Electron GUI mode, `hermes:startDashboard` delegates the visual mount to `hermes-dashboard-host`. |
...
The Playwright driver (~128 MB) is deliberately **not** in `hermes-venv`. It is
only used by the Google Meet bot, so `bundle-hermes.*` installs only
`websockets` (needed by Meet's remote-node mode) and leaves the Playwright
package + Chromium browser to be fetched on demand on the bot's first use. The
on-demand install (overlay patch `09-meet-playwright-on-demand`) targets an
app-scoped writable dir under `$HERMES_HOME/runtime-deps/google-meet`, so the
signed `vendor/hermes` runtime is never mutated. First use requires network; an
offline first use fails with a clear error instead of crashing.
