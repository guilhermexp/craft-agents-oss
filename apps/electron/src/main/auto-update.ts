 ...
 * electron-updater library. Updates are served from GitHub Releases of the fork
 * (provider config in apps/electron/electron-builder.yml).
 *
 * Platform behavior:
...
import { RPC_NAMESPACES, type UpdateInfo } from '../shared/types'
...
function broadcastUpdateInfo(): void {
  ...
  eventSink(RPC_NAMESPACES.update.AVAILABLE, { to: 'all' }, snapshot)
}
...
function broadcastDownloadProgress(progress: number): void {
  ...
  eventSink(RPC_NAMESPACES.update.DOWNLOAD_PROGRESS, { to: 'all' }, progress)
}
...
// Accept prerelease channel (rc.X tags) for stable clients too. Fork ships only
// prereleases today; without this, clients on stable semver would never see them.
