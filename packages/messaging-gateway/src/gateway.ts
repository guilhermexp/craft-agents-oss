...
export class MessagingGateway {
  ...
  private sweepStalePermissions(event: SessionEvent): void {
    ...
    for (const [requestId, record] of this.permissionMessages) {
      ...
      if (adapter?.clearButtons && adapter.isConnected()) {
        adapter.clearButtons(record.messagingChannelId, record.messageId).catch(() => {})
      }
  ...
  private async handleButtonPress(platform: PlatformType, press: ButtonPress): Promise<void> {
    ...
    if (press.buttonId.startsWith('bind:')) {
      ...
      if (!session) {
        await adapter.sendText(press.messagingChannelId, 'Session not found.', pressOpts)
        return
      }
    ...
    if (press.buttonId.startsWith('perm:')) {
      if (platform === 'whatsapp') {
        this.log.warn('ignored chat-side permission interaction for WhatsApp', {
          ...
          messagingChannelId: press.messagingChannelId,
  ...
  private async handlePermissionButton(
    ...
    if (adapter.clearButtons) {
      await adapter.clearButtons(record.messagingChannelId, record.messageId).catch(() => {})
    }
    ...
    await adapter.sendText(press.messagingChannelId, allowed ? '✅ Allowed' : '❌ Denied', pressOpts)
  }
  ...
  private async handlePlanButton(
    ...
    if (action === 'accept') {
      try {
        ...
        await adapter.sendText(press.messagingChannelId, '✅ Plan accepted. Agent resuming.', pressOpts)
      } catch (err) {
    ...
    const binding = this.bindingStore.findByChannel(platform, press.messagingChannelId, press.threadId)
    if (!binding) return
  ...
  private async finishPendingCompactAccept(sessionId: string): Promise<void> {
    ...
    try {
      ...
      if (adapter?.isConnected()) {
        await adapter.sendText(entry.messagingChannelId, '✅ Plan executing after compaction.', opts)
      }
    } catch (err) {
