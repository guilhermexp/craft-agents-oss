 ...
 * TurnCard.tsx
 *
 * The turn shell: an email-like card for one assistant turn. Owns turn-level
 * expansion, the collapsed preview + activity toggle, and lays out the activity
 * tree (./activity-tree), plans / questions and the response card
 * (./response-card). Shared value types + SIZE_CONFIG live in ./turn-card-shared.
 */
import * as React from 'react'
import { useMemo, useEffect, useRef, useCallback, useState } from 'react'
import type { AnnotationV1, AskUserQuestionItem, AskUserQuestionResponse } from '@craft-agent/core'
...
import { SIZE_CONFIG, type ActivityItem, type TodoItem, type ResponseContent, type AnnotationInteractionMode, type OpenAnnotationRequest, type GroupExpansionController } from './turn-card-shared'
import { getPreviewText } from './tool-descriptor'
import { isResponseBuffering } from './streaming-buffer'
import { ActivityRow, ActivityGroupRow, TodoList } from './activity-tree'
...
export interface TurnCardProps {
  ...
  /**
   * Resolved, single-polarity activity-group expansion controller. When omitted,
   * the card manages group expansion locally (uncontrolled, default collapsed).
   */
...
export const TurnCard = React.memo(function TurnCard({
  ...
  // Turn-level expansion: controlled via isExpanded/onExpandedChange, else local.
  const [localExpanded, setLocalExpanded] = useState<boolean>(defaultExpanded)
  const isExpanded = externalIsExpanded ?? localExpanded
  ...
  // Resolve one group-expansion controller: the provided one, else a local
  // uncontrolled fallback (default collapsed). No polarity re-derivation here —
  // the controller already returns final booleans.
  const [localGroups, setLocalGroups] = useState<Set<string>>(new Set())
  const resolvedGroupExpansion = useMemo<GroupExpansionController>(
    () => groupExpansion ?? {
      isExpanded: (groupId) => localGroups.has(groupId),
      setExpanded: (groupId, expanded) => setLocalGroups(prev => {
  ...
  const fullWidthCards = useMemo(
    () => allSortedActivities.filter(
      a => a.type === 'plan' || (a.type === 'tool' && isRenderableQuestion(a))
    ),
    [allSortedActivities]
  )
  ...
  const sortedActivities = useMemo(
    () => allSortedActivities.filter(a => a.type !== 'plan' && !(a.type === 'tool' && isRenderableQuestion(a))),
    [allSortedActivities]
  )
  ...
  return (
    ...
    <div className="space-y-1">
                    ...
                    groupedActivities.map((item, index) => (
                      isActivityGroup(item) ? (
                        <ActivityGroupRow
                          key={item.parent.id}
                          group={item}
                          groupExpansion={resolvedGroupExpansion}
      ...
      {fullWidthCards.map((activity, index) => {
        ...
        if (!isRenderableQuestion(activity)) return null
            ...
            <AskUserQuestionCard
              ...
              onRespond={
                onRespondToUserQuestion && activity.toolUseId
                  ? (response) => {
                      return onRespondToUserQuestion(activity.toolUseId!, response)
                    }
                  : undefined
              }
            />
  ...
  if (prev.groupExpansion !== next.groupExpansion) return false
...
// Public surface historically served by the `./chat/TurnCard` package subpath.
// These symbols moved out of this file during the module split; re-export them
// here so the subpath keeps publishing them (no consumer-facing narrowing).
