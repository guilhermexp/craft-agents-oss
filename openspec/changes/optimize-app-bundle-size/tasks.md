...
- [ ] 1.1 Registrar tamanho do `.app` atual: `bun run electron:dist:dev:mac` + `du -sh` do `.app` e de `Contents/Resources/app/{vendor,dist,resources}` (baseline = 981 MB)
...
- [ ] 2.3 Verificar no `.app` empacotado que `uv` existe só uma vez e o resolver ainda o encontra
...
- [ ] 3.2 Confirmar que o `electron-builder` ainda monta o DMG com o background (usa `resources/` como buildResources) e que os assets não estão em `Contents/Resources/app`
...
- [ ] 4.3 Verificar no `.app` que cada asset embarca uma vez e o app abre/roda normal
...
- [ ] 5.3 Confirmar boot do app empacotado com main minificado (sem regressão funcional)
...
- [ ] 6.2 Confirmar highlight de várias linguagens (comuns + exóticas: cpp, wasm, wolfram) renderizando com cor
- [ ] 6.3 Medir redução no bundle inicial do renderer
...
- [ ] 9.1 `bun run electron:dist:dev:mac` + `du -sh` do `.app`: comparar com baseline 981 MB e registrar delta real
- [ ] 9.2 `bun run validate:ci` verde (typecheck + testes shared/config/doc-tools + i18n parity)
- [ ] 9.3 Smoke real do app empacotado: abre, sessão roda, highlight de código, bot do Meet on-demand
- [ ] 9.4 `openspec validate optimize-app-bundle-size --strict --no-interactive` verde
