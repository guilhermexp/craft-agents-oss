...
- **A denied tool must not end the Claude turn.** In the Claude Agent SDK a `PreToolUse` hook returning `continue: false` blocks the tool **and** stops the agent loop right after the hook — the model never reads the reason and the user is left with a red tool card and no assistant text (`Session … completed without assistant response` is the misleading symptom, not context overflow). Every recoverable block therefore encodes as `{ continue: true, hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision: 'deny', permissionDecisionReason } }`: prerequisite/guide blocks, permission-mode denials, source activation (success **and** failure), `AskUserQuestion` arg validation, the >5MB image guard, and "no permission handler available". Turn termination is opt-in via `ToolPermissionResult`'s `block.endTurn`, set at exactly one site — a permission prompt that came back denied (`core/tool-permission-dispatcher.ts`), which is normally the user answering *no* but also covers `clearPendingPermissions()` during forceAbort/destroy and the broker's fail-closed rejection in `SessionManager.respondToPermission` — and the encoder then returns `{ continue: false, decision: 'block', reason, stopReason }` so the dead turn is explainable. `blockWithReason` (`mode-manager.ts`) owns the `[ERROR] ` marker for `isError` blocks and `encodeClaudeToolBlock` (`claude-agent.ts`) reads the marked text back off it, keeping the marker contract coherent with `session-tools-core/src/response.ts:errorResponse` and the strip in `ui/src/components/chat/turn-utils.ts`. Pi is unaffected: it answers with `tool_execute_response`. Guarded by `agent/__tests__/claude-tool-block-encoding.test.ts` and `agent/core/__tests__/tool-permission-dispatcher.test.ts`.
- **A pending steer rides the same hook output, deny included.** `canDeliverSteer()` (`claude-agent.ts`) decides who carries `pendingSteerMessage` as `hookSpecificOutput.additionalContext`: every outcome that keeps the turn alive — `allow`, `modify`, `passthrough` and a `deny` without `endTurn` (`encodeClaudeToolBlock(result, steerContext)`). The `endTurn` shape must **not** consume it: the loop dies right after the hook, and leaving it pending is what makes the turn emit `steer_undelivered` so the session layer re-queues the message. **Ordering is load-bearing** — `withUndeliveredSteer()` wraps the turn generator and emits `steer_undelivered` *before* the turn's `complete`, because `SessionManager.sendMessage` returns from its `for await` on the first `complete` and a `yield` reached afterwards (from a `finally`, as the pre-fix shape did) is discarded by the abandoned loop — the user's mid-turn message was lost, not re-queued. Consuming the steer before the switch (the older shape) dropped it on every block, and with the turn now surviving N tool calls that is the common path, not a rare one.
- **The prerequisite deadlock escape is per turn.** `PrerequisiteManager.MAX_REJECTIONS` (`agent/core/prerequisite-manager.ts`) was calibrated when one block ended the turn, so releasing on the second call meant a human had resent the message. With the turn alive the model emits both calls itself, so the budget is now 3 **and** re-armed by `beginTurn()` at the top of `BaseAgent.chat()` (Claude and Pi). Only the *unspent* budget re-arms: `readFiles`, `pendingSkillPaths` and `releasedPaths` are session-lived and dropped solely by `resetReadState()` on compaction. A path the escape already conceded goes into `releasedPaths` and is never charged again — otherwise the per-turn re-arm turns a one-time escape into `MAX_REJECTIONS` blocked calls at the start of *every* turn, for a condition (guide unreadable, file gone, Read disabled) that cannot resolve itself. The escape exists for the model that *cannot* comply (guide unreadable/vanished, Read disabled) — denying forever burns the turn in a tool-call loop with no assistant text. `strict` rules (browser docs) never reach the fallback, and the skill arm charges the budget to one pending path and releases only that path instead of `pendingSkillPaths.clear()`. Guarded by `agent/core/__tests__/prerequisite-manager.isolated.ts`.
- A turn that ends without an assistant message is never silent **unless the turn was actually cut short**: `SessionManager`'s `complete` handler (`packages/server-core/src/sessions/SessionManager.ts`) keeps the captured-400 `image_too_large`/`invalid_request` path and otherwise emits a generic retryable `unknown_error` ("No Response"), but only when `shouldReportMissingAssistantResponse(managed)` agrees. It takes the session, not loose flags, and suppresses only on `stopRequested` (Stop already renders "Response interrupted"; `cancelProcessing` leaves `isProcessing` true so events drain, so the stop signals are still set here) or `wasInterrupted` (a real abort — Stop, or a `steer` send whose `redirect()` failed and force-aborted the turn). A **queued** message suppresses nothing: `'queue'` is the anthropic default, nothing is aborted, the turn runs to natural completion, and hiding the card there hides exactly the silent failure this branch exists to expose. "Did this turn answer?" is read off `turnStartFinalMessageId`, not the caller's message timestamps, because a mid-stream user message is newer than the reply of the turn it arrived in. The SDK `result.stop_reason` is **not** plumbed into that branch — `AgentEvent`'s `complete` variant (`packages/core/src/types/message.ts`) carries no reason field, so a per-turn stop reason would need a new field across core types, the Claude event adapter and the session layer. It also suppresses two silences that are not failures at all: a turn whose history already carries a `role: 'error'` message newer than `turnStartLastMessageId` (the real diagnosis — rate limit, overloaded, credit balance, auth — was already rendered, and stacking a generic `canRetry: true` card on top of a `canRetry: false` error is the exact failure-hiding this branch exists to kill; an error from a PRIOR turn must still report), and a turn flagged `turnCompacted` (a successful `/compact` runs as an ordinary turn producing only info/status + `complete`, with no stop or abort signals). Both fields are runtime-only, set where `turnStartFinalMessageId` is set and cleared where it is cleared. Guarded by `packages/server-core/src/sessions/no-response-guard.isolated.ts`.
