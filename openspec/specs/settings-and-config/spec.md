# settings-and-config Specification

## Purpose
Cobrir o conjunto de settings tabs (env, messengers, skills, logs, AI, input, labels, permissions, server, workspace, Hermes) com persistência imediata por mudança, tema light/dark/system aplicado live, preferências escopadas por usuário/workspace, validação de credenciais (LLM connections) antes de persistir, e migrations de storage versionadas e idempotentes que bloqueiam boot se falharem.
## Requirements
