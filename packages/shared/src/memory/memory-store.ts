...
import { openSQLite, type SQLiteDatabase } from './sqlite-driver.ts';
...
export class MemoryStore {
  ...
  constructor(dbPath?: string) {
    this.db = openSQLite(dbPath ?? ':memory:');
  ...
  async upsert(params: {
    ...
    if (existing) {
      ...
      const updated = this.db.prepare('SELECT * FROM memories WHERE id = ?').get(existing.id) as MemoryRow;
    ...
    if (params.refIds?.length) {
      const insertRef = this.db.prepare('INSERT OR IGNORE INTO memory_refs (source_id, target_id) VALUES (?, ?)');
    ...
    const memory = rowToMemory(
      this.db.prepare('SELECT * FROM memories WHERE id = ?').get(id) as MemoryRow,
      params.tags ?? [],
      params.refIds ?? [],
    );
  ...
  searchFTS(query: string, options?: {
    ...
    const rows = this.db.prepare(sql).all(...sqlParams) as (MemoryRow & { rank: number })[];
  ...
  getByTarget(target: MemoryTarget, category?: MemoryCategory): Memory[] {
    ...
    const rows = this.db.prepare(sql).all(...sqlParams) as MemoryRow[];
  ...
  hasTurnBeenProcessed(turnId: string): boolean {
    const row = this.db.prepare('SELECT turn_id FROM processed_turns WHERE turn_id = ?').get(turnId);
    return row != null;
  }

  markTurnProcessed(turnId: string): void {
    this.db.prepare('INSERT OR IGNORE INTO processed_turns (turn_id, processed_at) VALUES (?, ?)').run(turnId, Date.now());
  }
  ...
  pruneAndArchive(thresholdSalience: number): number {
    const now = Date.now();
    const all = this.db.prepare('SELECT * FROM memories').all() as MemoryRow[];
    let pruned = 0;
  ...
  private getTagsForMemory(memoryId: string): string[] {
    const rows = this.db.prepare('SELECT tag FROM memory_tags WHERE memory_id = ?').all(memoryId) as { tag: string }[];
    return rows.map(r => r.tag);
  }

  private getRefsForMemory(memoryId: string): string[] {
    const rows = this.db.prepare('SELECT target_id FROM memory_refs WHERE source_id = ?').all(memoryId) as { target_id: string }[];
