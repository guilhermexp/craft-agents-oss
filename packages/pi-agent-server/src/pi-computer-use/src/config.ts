...
// Craft runtime overlay: mirror the Pi SDK's agent-dir resolution without a
// runtime SDK import so the isolated extension remains self-contained.
function getAgentDir(): string {
	...
	if (configuredDir.startsWith("~/")) return path.join(os.homedir(), configuredDir.slice(2));
