...
- [Embedded Hermes runtime](./docs/hermes-embed.md) — bundled Python venv, ACP stdio, seed skills, release packaging, signing, and lifecycle hardening.
- [Channels / War Room](./docs/channels-war-room.md) — shared channel model, Hermes orchestration, mentions, and Kanban integration.
- [Hermes seed resources](./resources/hermes-seed/README.md) — repository-bundled Craft knowledge copied into app-scoped Hermes homes on first launch.
...
### Embedded Hermes release contract

A release build must include the embedded Hermes runtime and the repository
seed resources. The macOS package layout is:

```txt
Craft Agents.app/Contents/Resources/app/
├── vendor/hermes/
│   ├── hermes-venv/bin/python3
│   ├── hermes-agent/acp_adapter/server.py
│   └── bin/rg
└── dist/resources/hermes-seed/manifest.json
```

Important rules:

- `resources/vendor/hermes/` is generated by `bun run bundle:hermes` and is
  copied into the packaged app through `electron-builder.yml` `extraResources`
  as `app/vendor/hermes`.
...
- Build one Hermes architecture at a time. Use `dist:mac`/`release:mac` for
  arm64 and `dist:mac:x64`/`release:mac:x64` for Intel; do not hard-code both
  archs in `electron-builder.yml` while the Hermes runtime is arch-specific.
- `scripts/build-dmg.sh` and `scripts/build/darwin.ts` intentionally fail closed
  if the packaged app is missing Hermes Python, `acp_adapter/server.py`, or the
  `hermes-seed` manifest.

Manual verification after a local macOS build:

```bash
APP="apps/electron/release/mac-arm64/Craft Agents.app"
test -x "$APP/Contents/Resources/app/vendor/hermes/hermes-venv/bin/python3"
test -f "$APP/Contents/Resources/app/vendor/hermes/hermes-agent/acp_adapter/server.py"
test -f "$APP/Contents/Resources/app/dist/resources/hermes-seed/manifest.json"
test ! -e "$APP/Contents/Resources/app/dist/resources/vendor/hermes"
"$APP/Contents/Resources/app/vendor/hermes/hermes-venv/bin/python3" - <<'PY'
import acp_adapter.server
import hermes_cli
print('packaged hermes ok')
PY
...
- `electron-builder.yml` excludes any `.venv/**` directory — packaging would
  otherwise choke on relative Python symlinks from dev-only virtualenvs.
- Hermes' release runtime is under `resources/vendor/hermes/`, not a sibling
  `resources/vendor/hermes-agent/.venv`. Do not restore old docs/scripts that
  expect a standalone Hermes checkout or a `.venv` in app resources.
