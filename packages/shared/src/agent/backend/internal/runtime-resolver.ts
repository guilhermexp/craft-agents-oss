...
function resolveClaudeBinaryPath(hostRuntime: BackendHostRuntimeContext): string | undefined {
  ...
  if (IS_DEV_RUNTIME) {
    const upwards = resolveUpwards(hostRuntime.appRootPath, aliasRel, 10)
      ?? (platformRel ? resolveUpwards(hostRuntime.appRootPath, platformRel, 10) : undefined);
    if (upwards) return upwards;
  }

  // Fork adaptation (bun): in non-packaged runs (electron:dev, headless server)
  // bun installs the per-platform optional-dep into its own `.bun` store and
  // does NOT lay down a flat `node_modules/@anthropic-ai/<pkg>` directory, so the
  // hardcoded candidates above miss it. Resolve it the same way `sdk.mjs` does —
  // relative to the main `claude-agent-sdk` package (its optionalDependencies
  // parent) — and derive the sibling `claude` binary. Skipped when packaged: a
  // packaged app must ship the staged binary at one of the candidates above, and
  // the bundle's node_modules layout isn't a reliable module-graph source.
  if (pkg && !hostRuntime.isPackaged) {
    return resolveBinaryViaSdkPackage(`@anthropic-ai/${pkg}`, binaryName);
  }
  return undefined;
}

/**
 * Resolve `<binaryName>` next to the per-platform SDK package, locating that
 * package from the main `@anthropic-ai/claude-agent-sdk` root (its declared
