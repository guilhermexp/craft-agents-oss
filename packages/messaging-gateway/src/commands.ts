...
export class Commands {
  ...
  private async handleNew(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    ...
    try {
      ...
      await adapter.sendText(msg.messagingChannelId, `Failed to create session: ${errorMsg}`, replyOpts)
    }
  }
  ...
  private async handleBind(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    ...
    if (bindArg) {
      ...
      if (!session) {
        await adapter.sendText(msg.messagingChannelId, `Session not found: ${bindArg}`, replyOpts)
        return
      }
      ...
      await adapter.sendText(msg.messagingChannelId, `Bound to "${session.name || session.id}"`, replyOpts)
      return
    }
  ...
  private async handlePair(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    ...
    if (!this.pairingConsumer) {
      await adapter.sendText(msg.messagingChannelId, 'Pairing is not available in this build.', replyOpts)
      return
    }
    ...
    if (!entry) {
      await adapter.sendText(msg.messagingChannelId, 'Invalid or expired pairing code.', replyOpts)
      return
    }
    ...
    if (!session) {
      await adapter.sendText(msg.messagingChannelId, 'Session no longer exists.', replyOpts)
      return
    }
  ...
  private async handleUnbind(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    const replyOpts = msg.threadId !== undefined ? { threadId: msg.threadId } : {}
    const removed = this.bindingStore.unbind(adapter.platform, msg.messagingChannelId, msg.threadId)
    if (removed) {
      await adapter.sendText(msg.messagingChannelId, 'Disconnected from session.', replyOpts)
    } else {
      await adapter.sendText(msg.messagingChannelId, 'No session is bound to this chat.', replyOpts)
    }
  }

  private async handleStatus(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    const replyOpts = msg.threadId !== undefined ? { threadId: msg.threadId } : {}
    const binding = this.bindingStore.findByChannel(adapter.platform, msg.messagingChannelId, msg.threadId)
    if (!binding) {
      await adapter.sendText(msg.messagingChannelId, 'No session bound. Use /bind, /new, or /pair.', replyOpts)
      return
    }
  ...
  private async handleStop(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    ...
    const binding = this.bindingStore.findByChannel(adapter.platform, msg.messagingChannelId, msg.threadId)
    if (!binding) {
      await adapter.sendText(msg.messagingChannelId, 'No session bound.', replyOpts)
      return
    }

    try {
      await this.sessionManager.cancelProcessing(binding.sessionId)
      await adapter.sendText(msg.messagingChannelId, 'Stopped.', replyOpts)
    } catch {
      await adapter.sendText(msg.messagingChannelId, 'Nothing to stop.', replyOpts)
    }
  }
