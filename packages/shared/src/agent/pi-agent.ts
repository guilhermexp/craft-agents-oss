...
// Copilot token exchange (Pi SDK ≥0.80.8 removed the pi-ai/oauth runtime export)
import { refreshGitHubCopilotToken } from './backend/pi/copilot-oauth.ts';
...
export class PiAgent extends BaseAgent {
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      // session_state, source state, memory) ride the user-message tail so a
      // per-turn re-stamp doesn't invalidate the prompt cache.
      // buildVolatileContextParts consumes the one-shot mode-change signal, so it
